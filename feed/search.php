...
$searchword= isset($_GET['search']) ? mysqli_real_escape_string($conn, $_GET['search']) : '';
        ...
        function joinCommunity(communityName) {
            ...
            xhr.onreadystatechange = function() {
                if (xhr.readyState === 4 && xhr.status === 200) {
                    // Optionally show feedback or update button
...
<body>
    ...
    <div class="navbar">
        <a href="search.php?page=accounts<?php echo isset($_GET['search']) ? '&search=' . urlencode($_GET['search']) : ''; ?>" class="nav-btn<?php echo (isset($_GET['page']) && $_GET['page'] === 'accounts') ? ' active' : ''; ?>" id="btn-accounts">Accounts</a>
        <a href="search.php?page=posts<?php echo isset($_GET['search']) ? '&search=' . urlencode($_GET['search']) : ''; ?>" class="nav-btn<?php echo (isset($_GET['page']) && $_GET['page'] === 'posts') ? ' active' : ''; ?>" id="btn-posts">Posts</a>
        <a href="search.php?page=ads<?php echo isset($_GET['search']) ? '&search=' . urlencode($_GET['search']) : ''; ?>" class="nav-btn<?php echo (isset($_GET['page']) && $_GET['page'] === 'ads') ? ' active' : ''; ?>" id="btn-ads">Ads</a>
    </div>
...
while ($row = mysqli_fetch_assoc($getrandom3)) {
    ...
    $random_picture = $row['picture'] ? $row['picture'] : 'default_profile_pic.png'; // Fallback to default if no profile pic
    echo "
